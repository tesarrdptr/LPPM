...
public partial class Page_Master_Penelitian : System.Web.UI.Page
{
    ...
    {
        gridData.DataSource = lib.CallProcedure("stp_getHistoryPengabdianNP", new string[] { Server.HtmlEncode(txtCari.Text.Trim()) });
        gridData.DataBind();
        hideAlert();
    }
    ...
    protected void btnSubmitAdd_Click(object sender, EventArgs e)
    {
        ...
        if (addUpload.PostedFile != null && addUpload.PostedFile.ContentLength > 0 && fileExtension == ".pdf")
        {
            ...
            if (contenttype != String.Empty)
            {
                ...
                param[7] = Server.HtmlEncode(AddKetua.Text.Trim());
                param[8] = Server.HtmlEncode(addAnggota.Text.Trim());
                param[9] = Server.HtmlEncode("1");
                param[10] = Context.User.Identity.Name;
                param[11] = Server.HtmlEncode(atc.Rows[0][0].ToString());
                param[12] = Server.HtmlEncode("1");
    ...
    {
        return lib.CallProcedure("stp_getHistoryPengabdian", new String[] { Context.User.Identity.Name, Server.HtmlEncode(txtCari.Text.Trim()) });
    }

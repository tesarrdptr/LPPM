...
public partial class Page_Master_Artikel_Prosiding : System.Web.UI.Page
{
   ...
   new SqlConnection(WebConfigurationManager.ConnectionStrings["LP2MConnection"].ConnectionString);
    ...
    {
        AddPenulis.DataSource = master.sso.CallProcedure("stp_getUserPeneliti", new String[] { Context.User.Identity.Name });
        AddPenulis.DataValueField = "iduser";
        AddPenulis.DataTextField = "iduser";
        AddPenulis.DataBind();
        AddPenulis.Items.Insert(0, new ListItem("--- Pilih Penulis ---", ""));
        ddlAddJenis.Items.Add(new ListItem("--- Pilih Jenis Prosiding ---", ""));
        ddlAddJenis.Items.Add(new ListItem("Tidak Diketahui", "Tidak Diketahui"));
        ddlAddJenis.Items.Add(new ListItem("Terindeks ", "Terindeks"));
        ddlAddJenis.Items.Add(new ListItem("Tidak Terindeks", "Tidak Terindeks"));
    }

    protected void loadData()
    {
        gridData.DataSource = lib.CallProcedure("stp_getHistoryAP", new string[] { Server.HtmlEncode(txtCari.Text.Trim()) });
        gridData.DataBind();
        hideAlert();
    }
    ...
    protected void gridData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "Ubah")
        {
            ...
            connection.Open();
            SqlCommand sqlCmd = new SqlCommand("stp_detaiAP", connection);
            ...
            while (dr.Read())
            {
                dtJudul.Text = dr["ap_judul"].ToString();
                dtNama.Text = dr["ap_nama"].ToString();
                dtJenis.Text = dr["ap_jenis"].ToString();
                dtPenulis.Text = dr["ap_penulis"].ToString();
                dtTahun.Text = dr["ap_tahun"].ToString();
                dtNomor.Text = dr["ap_nomor"].ToString();
                dtTglMulai.Text = dr["ap_tgl_mulai"].ToString();
                dtTglSelesai.Text = dr["ap_tgl_selesai"].ToString();
                dtVolume.Text = dr["ap_volume"].ToString();
                dtURL.Text = dr["ap_url"].ToString();
                creby.Text = dr["ap_creaby"].ToString();
                credate.Text = dr["ap_creadate"].ToString();
    ...
    protected void btnSubmitAdd_Click(object sender, EventArgs e)
    {
        ...
        if (addUpload.PostedFile != null && addUpload.PostedFile.ContentLength > 0 && fileExtension == ".pdf")
        {
            ...
            if (contenttype != String.Empty)
            {
                ...
                param[3] = Server.HtmlEncode(AddPenulis.SelectedValue.Trim());
                param[4] = Server.HtmlEncode(addVolume.Text.Trim());
                param[5] = Server.HtmlEncode(addNomor.Text.Trim());
                param[6] = Server.HtmlEncode(ddlAddJenis.SelectedValue.Trim());
                ...
                param[10] = Server.HtmlEncode(tglMulai1.Text.Trim());
                param[11] = Server.HtmlEncode(tglSelesai.Text.Trim());

                try
                {
                    lib.CallProcedure("stp_createHistoryAP", param);
                    loadData();
    ...
    {
        return lib.CallProcedure("stp_getHistoryAP", new String[] { Context.User.Identity.Name, Server.HtmlEncode(txtCari.Text.Trim()) });
    }

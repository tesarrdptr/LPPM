...
public partial class Page_Master_Buku : System.Web.UI.Page
{
    ...
    {
        gridData.DataSource = lib.CallProcedure("stp_getHistoryBuku", new string[] { Server.HtmlEncode(txtCari.Text.Trim()) });
        gridData.DataBind();
        hideAlert();
    }
    ...
    protected void linkTambah_Click(object sender, EventArgs e)
    {
        ...
        AddJudul.Text = " ";
        AddPenerbit.Text = " ";
        addPenulis.SelectedValue = "";
        addEditor.Text = " ";
        addHalaman.Text = " ";
        addUrl.Text = " ";
        ddlTahun.SelectedValue = "2019";
    }
    ...
    protected void gridData_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "Detil")
        {
            ...
            connection.Open();
            SqlCommand sqlCmd = new SqlCommand("stp_detailBuku", connection);
            ...
            while (dr.Read())
            {
                detilJudul.Text = dr["bk_judul"].ToString();
                detilPenerbit.Text = dr["bk_penerbit"].ToString();
                detilPenulis.Text = dr["bk_penulis"].ToString();
                detilEditor.Text = dr["bk_editor"].ToString();
                detilHalaman.Text = dr["bk_halaman"].ToString();
                ddlEditTahun.Text = dr["bk_tahun"].ToString();
                editUrl.Text = dr["bk_url"].ToString();
                creaby.Text = dr["bk_creaby"].ToString();
                creadate.Text = dr["bk_creadate"].ToString();
                updby.Text = dr["bk_updateby"].ToString();
                upddate.Text = dr["bk_updatedate"].ToString();
    ...
    protected void btnSubmitAdd_Click(object sender, EventArgs e)
    {
        ...
        if (addUpload.PostedFile != null && addUpload.PostedFile.ContentLength > 0 && fileExtension == ".pdf")
        {
            ...
            if (contenttype != String.Empty)
            {
                ...
                param[1] = Server.HtmlEncode(ddlTahun.SelectedValue.Trim());
                param[2] = Server.HtmlEncode(addHalaman.Text.Trim());
                param[3] = Server.HtmlEncode(AddPenerbit.Text.Trim());
                param[4] = Server.HtmlEncode(addPenulis.SelectedValue.Trim());
                param[5] = Server.HtmlEncode(addEditor.Text.Trim());
                param[6] = Server.HtmlEncode(addUrl.Text.Trim());
                param[7] = Context.User.Identity.Name;
                param[8] = Server.HtmlEncode(atc.Rows[0][0].ToString());

                try
                {
                    lib.CallProcedure("stp_createHistoryBuku", param);
                    loadData();
    ...
    {
        return lib.CallProcedure("stp_getHistoryBuku", new String[] { Context.User.Identity.Name, Server.HtmlEncode(txtCari.Text.Trim()) });
    }
